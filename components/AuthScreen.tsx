...
import { Mail, Chrome, Eye, EyeOff, User, Lock, ArrowLeft, Code } from 'lucide-react';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, onAdminLogin }) => {
  const { login, loginWithGoogle, register, loading: authLoading } = useAuth(); // Use hook
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // onLogin is handled by the App component listening to auth state changes, 
      // but we can call it here if we need to pass specific data not in context
    } catch (err: any) {
...
const handleQuickLogin = () => {
  // For testing - skip to character selection
  onLogin({ id: 'guest', username: 'Misafir' });
};
